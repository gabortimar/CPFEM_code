...
!------------------------------------read twinning probability
			READ(in) twinprob

!------------------------------------read elastic constants
			READ(in) (e(j, mat_code), j= 1, 9)

!------------------------------------read slip/twin system reorientation angles
			READ(in) (ssang(j, mat_code), j= 1, nss(mat_code))
...
!----------Initialise tflag, cumslip, twinsys and timestwinned variables:
	tflag = 0
	do i= 1, ngps
		do j= 1, nss(1)
			cumslip(j,i) = 0.
...
!***********************************************************************!
!																		!
!		SUBROUTINE WriteSlips												!
!-----------------------------------------------------------------------!
!		Writes slip rates for all slip systems to .slp files									!
!-----------------------------------------------------------------------!
!																		!
!***********************************************************************!
SUBROUTINE WriteSlips(f_name)
	...
	write(*,*) f_name
	open (unit=54,file=f_name,action="write",status="replace")

	do i=1, ngps
		write(54,'(9999ES15.5)') (cumslip(j,i),j=1,nss(1)), (srate(j,i),j=1,nss(1))
	end do

	close (54)

END SUBROUTINE WriteSlips
...
!------------------------------------write slip system reorientation angles
			WRITE(out) (ssang(j, mat_code), j= 1, nss(mat_code))
